...
if(file_exists("/public/assets/_logos/logo".$UGSELREG[0].".gif")) $LogoReg = $UGSELREG[0]; else	$LogoReg = "un";
...
<table bgcolor='aliceblue' bordercolor='darkblue' bordercolordark = 'darkblue' bordercolorlight = 'darkblue' border='2' rules ='none' cellpadding='0' cellspacing='0' width='100%' height='80%'>
    ...
    <tr><td Width = '35%' Rowspan = '<?php echo ($NbDep+2) ?>' Align = 'Center' Style = 'padding:0px 20px 0px 20px;'><font size = '4'><b>R�gion <?php echo $UGSELREG[1];?></b></font><br>
            <br>&nbsp; &nbsp;<a href="http://<?php echo $UGSELREG[4];?>" class="img-lien"><img src="/assets/_logos/logo<?php echo $LogoReg;?>.gif" height="<?php echo 130 //$UGSELREG[2];?>" width="<?php echo 130 //$UGSELREG[3];?>" alt =''>&nbsp;</a>
            <br><br><i><b>Bienvenue dans UGSEL Web<br><br>L'espace d'inscription aux comp�titions Ugsel</b><Font size = '2'><br><br><br>Acc�dez � l'Ugsel d�partementale de votre choix en cliquant sur son nom...</FONT></I>
    ...
    <?php for ($i = 0; $i < $NbDep; $i++){
        ...
        echo "<tr><td Class = 'tddetail' Width = '5%' Align = 'RIGHT'> <img src = '/assets/_logos/logo".$LogoDep.".gif'  height='".$UGSELDEP[$i][2]."' width='".$UGSELDEP[$i][3]."' alt =''></td>
		<td Class = 'tddetail' Width = '55%'>��";
        if(file_exists("../app/views/".$UGSELREG[0]."/ud".$UGSELDEP[$i][0]))
